...
export default function Sidebar({ currentView, onViewChange, onLogout, isMobileOpen, onMobileClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static left-0 top-0 h-full w-64 bg-gray-900 text-white p-6 z-50 transform transition ${
      ...
      >
          ...
          {items.map((i) => (
            <button
              ...
              className={`flex items-center w-full p-2 rounded ${
                currentView === i.id ? "bg-emerald-600" : "hover:bg-gray-800"
              }`}
            >
        ...
        <button
          onClick={onLogout}
          className="flex items-center w-full mt-4 p-2 rounded hover:bg-gray-800 "
        >
